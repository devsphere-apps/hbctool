...
def parse(f):
    ...
    # Segment 15: FunctionSourceTable (v90+)
    functionSourceTable = []
    for _ in range(header["functionSourceCount"]):
        functionSourceEntry = {}
        for key in functionSourceTableS:
            functionSourceEntry[key] = read(f, functionSourceTableS[key])
        functionSourceTable.append(functionSourceEntry)
...
def export(obj, f):
    ...
    # Segment 15: FunctionSourceTable (v90+)
    functionSourceTable = obj["functionSourceTable"]
    for i in range(header["functionSourceCount"]):
        functionSourceEntry = functionSourceTable[i]
        for key in functionSourceTableS:
            write(f, functionSourceEntry[key], functionSourceTableS[key])
